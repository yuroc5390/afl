...
static const char DSACertPEM[] = {
    ...
    if (comp_methods != NULL)    sk_SSL_COMP_sort(comp_methods);

	OpenSSL_add_all_algorithms();
	// setup client and server bio and contexts




//    ctx_server = SSL_CTX_new(SSLv23_method());
	ctx_server = SSL_CTX_new(SSLv23_server_method());
    ret = SSL_CTX_set_min_proto_version(ctx_server, 0);
    OPENSSL_assert(ret == 1);
    ret = SSL_CTX_set_cipher_list(ctx_server, "ALL:eNULL:@SECLEVEL=0");
    OPENSSL_assert(ret == 1);
    /* RSA */
    ...
    ret = SSL_CTX_use_PrivateKey(ctx_server, pkey);
    OPENSSL_assert(ret == 1);
    EVP_PKEY_free(pkey);
    bufp = kCertificateDER;
    cert = d2i_X509(NULL, &bufp, sizeof(kCertificateDER));
    OPENSSL_assert(cert != NULL);
    ret = SSL_CTX_use_certificate(ctx_server, cert);
    OPENSSL_assert(ret == 1);
    X509_free(cert);

    server = SSL_new(ctx_server);
    in_server = BIO_new(BIO_s_mem());
    out_server = BIO_new(BIO_s_mem());
    SSL_set_bio(server, in_server, out_server);
...
//    ctx_client = SSL_CTX_new(SSLv23_method());
    ...
    client = SSL_new(ctx_client);
    OPENSSL_assert(SSL_set_min_proto_version(client, 0) == 1);
    OPENSSL_assert(SSL_set_cipher_list(client, "ALL:eNULL:@SECLEVEL=0") == 1);
//    SSL_set_tlsext_host_name(client, "localhost");
    in_client = BIO_new(BIO_s_mem());
    out_client = BIO_new(BIO_s_mem());
    SSL_set_bio(client, in_client, out_client);
...
//
	...
	do{
		...
		r = BIO_read(out_client,buf,4096);
		if (r==-1) err();
		TPstate++;
		...
		if (TPstate == pstate) {
			...
			sprintf(fn, "packet-%i", TPstate);
			f = fopen(fn, "wb");
			fwrite(buf, 1, r, f);
			fclose(f);
		}
		BIO_write(in_server, buf, r);
		...
		r = BIO_read(out_server, buf, 4096);
		if (r == -1)  err();
		TPstate++;
		if (TPstate == pstate) {
#ifdef __AFL_HAVE_MANUAL_CONTROL
		...
		printf("server state: %s / %s\n", SSL_state_string(server),SSL_state_string_long(server));
		  ...
		  || !SSL_is_init_finished(client)) && TPstate < 10);
